  ...
  if (logos.length === 0) {
    return (
      <p className="text-center text-sm text-white/50">
        Add client logos to <code className="text-accent">src/assets/clients/</code>
      </p>
    );
  }
  ...
  return (
    <div className="space-y-12">
      ...
      >
        {logos.map((logo, index) => (
          <motion.div
            ...
            className="group relative flex h-40 w-64 items-center justify-center overflow-hidden rounded-3xl border border-white/20 bg-surface p-8 shadow-2xl shadow-primary/20 sm:h-48 sm:w-80 md:h-56 md:w-96 lg:h-64 lg:w-[28rem]"
          >
            ...
            />
            <p className="absolute bottom-4 left-0 right-0 z-20 text-center text-sm font-bold uppercase tracking-[0.2em] text-foreground/70">
              {logo.name}
            </p>
          </motion.div>
        ))}
      ...
      <div className="relative overflow-hidden py-4">
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-20 bg-gradient-to-r from-primary via-primary/80 to-transparent sm:w-32" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-20 bg-gradient-to-l from-primary via-primary/80 to-transparent sm:w-32" />
